...
if(gpsId == NULL || type == NULL || name == NULL || city == NULL || countryAbbrv == NULL || airport == NULL){
    return;
}
if(latitude < -90 || latitude > 90 || longitude < -180 || longitude > 180){
    return;
...
sprintf(string, "%s, %s, %s, %f, %f, %d, %s, %s,", a->gpsId, a->type, a->name, a->latitude, a->longitude, a->elevationFeet, a->city,
a->countryAbbrv);
